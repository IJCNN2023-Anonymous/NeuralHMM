...
def train_riskpre(train_loader,
    ...
    for each_epoch in range(epochs):
        ...
        if cur_auroc > best_auroc:
            ...
            best_epoch_train.extend([h_gru_train, mu_q_train, var_q_train, batch_x_train, batch_y_train, batch_name_train])
            best_epoch_valid.extend([h_gru_valid, mu_q_valid, var_q_valid, batch_x_valid, batch_y_valid, batch_name_valid])
            print('------------ Save best model ------------')

    return best_epoch, best_auroc, best_auprc, history, best_epoch_train, best_epoch_valid
...
if __name__ == '__main__':
    ...
    best_epoch, best_auroc, best_auprc, history, best_epoch_train, best_epoch_valid = train_riskpre(train_loader, valid_loader, idx_list, \
                  ...
                  gru_dropout_rate, hidden_dim_, out_dim, n_gaussian, lr, epochs, seed, device, model_name = 'RiskPre')
    ...
    min_loss = train_neuralhmm(idx_list, demographic_data, diagnosis_data, h_gru_train, h_gru_valid, mu_q_train, mu_q_valid, \
                    ...
                    h_dim, s_dim, emission_dim, out_dim, transition_dim_, transition_dim, lr, epochs, seed, device, model_name='NeuralHmm')
