...
class Predictor(nn.Module):
    ...
    def forward(self, h, s):
        ...
        mu = mu.view(-1, self.num_class, self.n_gaussian).permute(2, 0, 1)
        sigma = sigma.view(-1, self.num_class, self.n_gaussian).permute(2, 0, 1)
...
class RiskPre(nn.Module):
    """
        The Proposed Model
    """
    def __init__(self, input_dim, h_dim, s_dim, gru_dim, gru_num_layers, gru_dropout_rate, hidden_dim_, seq_len, out_dim, n_gaussian, device):
        ...
        self.predictor = Predictor(h_dim, s_dim, seq_len, out_dim, n_gaussian)
    ...
    def forward(self, x, s):
        """
            ...
            returns :
                sample_c : the predicted class probability distribution ( B x num_class )
                h_gru : a series of hidden states obtained from GRU ( B x T x gru_dim )
                mu_q : the mean of the approximated distribution ( B x h_dim )
                var_q :  the variance of the approximated distribution ( B x h_dim )
        """
        batch_size = x.size(0)
        t_max = x.size(1)
        ...
        return sample_c, h_gru, mu_q_seq, var_q_seq
